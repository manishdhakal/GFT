...
import timm.scheduler as lr_scheduler
from pathlib import Path
from tqdm import tqdm
from dataset import PartNormalDataset

from misc import summary_parameters
...
def seed_everything(seed=42):
    ...
    torch.backends.cudnn.deterministic = (
        True  # Make CUDA deterministic (may impact performance)
    )
    torch.backends.cudnn.benchmark = (
        False  # Disable benchmark to avoid random behavior in select cases
    )

def main(cfg_path="cfgs/gft.yaml"):
    def log_string(str):
        logger.info(str)
        print(str)

    # CLI just for config path
    ...
    # Merge with CLI overrides (from unknown args)
    ...
    if config.log_dir is None:
        ...
        exp_dir = exp_dir.joinpath(config.log_dir)
    exp_dir.mkdir(exist_ok=True)
    ...
    file_handler = logging.FileHandler(
        "%s/%s_%s.log" % (log_dir, config.model.type, timestr)
    )
    file_handler.setLevel(logging.INFO)
    file_handler.setFormatter(formatter)
    logger.addHandler(file_handler)
    log_string(f"========== CONFIGURATION ==========\n{OmegaConf.to_yaml(config)}")

    root = config.root

    TRAIN_DATASET = PartNormalDataset(
        root=root, npoints=config.npoint, split="trainval", normal_channel=config.normal
    )
    ...
    TEST_DATASET = PartNormalDataset(
        root=root, npoints=config.npoint, split="test", normal_channel=config.normal
    )
    ...
    MODEL = importlib.import_module(config.model.type)
    shutil.copy("models/%s.py" % config.model.type, str(exp_dir))
    # shutil.copy('models/pointnet2_utils.py', str(exp_dir))

    classifier = MODEL.get_model(config.model).cuda()

    criterion = MODEL.get_loss().cuda()
    classifier.apply(inplace_relu)
    print(
        "# generator parameters:",
        sum(param.numel() for param in classifier.parameters()),
    )
    start_epoch = 0

    if config.ckpts is not None:
        classifier.load_model_from_ckpt(config.ckpts)
    ...
    def add_weight_decay(model, weight_decay=1e-5, skip_list=(), optimizer_part="all"):
        ...
        for name, param in model.named_parameters():
            ...
            if optimizer_part == "gft":
                if (
                    "cls" in name
                    or "edge_conv" in name
                    or "cross_attn" in name
                    or "prompt" in name
                    or ("encoder.first_conv" in name)
                ):
                    if (
                        len(param.shape) == 1
    ...
    param_groups = add_weight_decay(
        classifier, weight_decay=0.05, optimizer_part=config.optimizer_part
    )

    # Get the optimizer class from the config
    optimizer_class = getattr(optim, config.optimizer.type)

    # Remove 'type' key before passing as kwargs and initialize the optimizer
    optimizer_kwargs = OmegaConf.to_container(config.optimizer, resolve=True)
    optimizer_kwargs.pop("type")
    ...
    # Remove 'type' key before passing as kwargs and initialize the scheduler
    scheduler_kwargs = OmegaConf.to_container(config.scheduler, resolve=True)
    ...
    for epoch in range(start_epoch, config.epoch):
        ...
        log_string("Epoch %d (%d/%s):" % (global_epoch + 1, epoch + 1, config.epoch))
        ...
        for i, (points, label, target) in tqdm(
            ...
            seg_pred = classifier(points, label)
            seg_pred = seg_pred.contiguous().view(-1, num_part)
            target = target.view(-1, 1)[:, 0]
            pred_choice = seg_pred.data.max(1)[1]

            correct = pred_choice.eq(target.data).cpu().sum()
            mean_correct.append(correct.item() / (config.batch_size * config.npoint))
