...
def rand_sample_points(points: torch.Tensor, num_samples: int) -> torch.Tensor:
    """
    Randomly samples unique points for each member of batch along the N dimension of a B x N x D tensor.

    Args:
        points (torch.Tensor): Input tensor of shape (B, N, D).
        num_samples (int): Number of points to sample (M).

    Returns:
        torch.Tensor: Tensor of shape (B, num_samples, D) with sampled points.
    """
    B, N, D = points.shape  # Get dimensions of the input tensor
    if num_samples > N:
        raise ValueError("num_samples cannot be greater than the N dimension of the input tensor.")

    # Create a tensor to store sampled points
    sampled_points = torch.empty((B, num_samples, D), dtype=points.dtype, device=points.device)

    for b in range(B):
        # Generate unique random indices for each batch
        indices = torch.randperm(N)[:num_samples]
        sampled_points[b] = points[b, indices, :]
