...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    wandb_run = wandb.init(project='GFT', name=f"{args.log_name}_{args.experiment_path.split('/')[-1]}", config=config)
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        wandb_run.log({"epoch": epoch, "train/loss": losses.avg(0), "val/acc": metrics.acc})
        # if (config.max_epoch - epoch) < 10:
        #     builder.save_checkpoint(base_model, optimizer, epoch, metrics, best_metrics, f'ckpt-epoch-{epoch:03d}', args, logger = logger)

    wandb_run.log({"val/best_acc": best_metrics.acc})
    wandb_run.finish()
